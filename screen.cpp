...
vector<int> amounts = {
    1, 5, 10, 25, 50, 75, 100, 200, 300, 400, 500,
...
void initializeSDL() {
    ...
    font = TTF_OpenFont("assets/Hanson-Bold.ttf", 20);
    if (font == nullptr) {
...
void renderCases(SDL_Texture* caseTexture) {
    ...
    int startY = (SCREEN_HEIGHT - (5 * (CASE_SIZE + CASE_MARGIN))) / 2;
    ...
    for (int i = 0; i < 26; ++i) {
        ...
        // Render the case texture
        SDL_RenderCopy(renderer, caseTexture, nullptr, &caseRect);

        // Render the case number
        renderText(to_string(i + 1), x + CASE_SIZE / 4, y + CASE_SIZE / 4, { 0, 0, 0, 255 });
    }
}
...
void renderAmounts() {
    ...
    for (size_t i = 0; i < amounts.size(); ++i) {
        ...
        int y = startY + i * 30;
        if (i >= amounts.size() / 2) {
            x = SCREEN_WIDTH - AMOUNT_MARGIN - 100;
